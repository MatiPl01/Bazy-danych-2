...
BEGIN
    ...
    WHEN 'c' THEN
        NULL;
    WHEN 'n' THEN
        RAISE_APPLICATION_ERROR(-20000, 'Cannot change the status of the reservation with id ' ||
    ...
    WHEN 'p' THEN
        ...
        IF l_curr_status = 'c' THEN
            ...
            IF l_available_places < l_no_places THEN
                RAISE_APPLICATION_ERROR(-20000, 'Not enough places available to update the cancelled reservation status');
            END IF;
        END IF;
    ELSE
        RAISE_APPLICATION_ERROR(-20000, 'Status: ' || p_status || ' is not a valid reservation status');
    END CASE;
    ...
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            RAISE_APPLICATION_ERROR(-20000, 'There is no reservation with id ' || p_reservation_id || ' in the database');
...
BEGIN
   modifyReservationStatus(11, 'p');
END;

-- [Error] Invalid reservation status
BEGIN
   modifyReservationStatus(11, 'z');
END;
...
BEGIN
    ...
    IF p_no_places <= 0 THEN
        RAISE_APPLICATION_ERROR(-20007, 'The number of booked places should be greater than 0');
    ELSIF p_no_places - l_curr_no_places > l_available_places THEN
        RAISE_APPLICATION_ERROR(-20007, 'There are not enough free places. Max possible number of places to book: ' ||
...
BEGIN
    ...
    IF p_max_no_places < l_booked_places THEN
        RAISE_APPLICATION_ERROR(-20007, 'The maximum number of places (' || p_max_no_places ||
